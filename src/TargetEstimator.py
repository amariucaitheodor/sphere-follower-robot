...
class TargetEstimator:
    ...
    def __init__(self):
        ...
        self.cam1_ratio_sub = rospy.Subscriber("/camera1/pix_to_m_ratio", Float64, self.callback_update_camera1_ratio, queue_size=10)
        self.cam2_ratio_sub = rospy.Subscriber("/camera2/pix_to_m_ratio", Float64, self.callback_update_camera2_ratio, queue_size=10)
    ...
    def callback_update_camera1_ratio(self, ratio):
        self.pix_to_m_ratio_img1 = ratio.data
        # print(self.pix_to_m_ratio_img1, " ratio 1")

    def callback_update_camera2_ratio(self, ratio):
    ...
    def image1_callback(self, data):
        ...
        self.target_position.data[2] = self.pix_to_m_ratio_img1 * ((np.absolute(self.target_history[2] - base_frame[1]) + sphere_relative_distance[1]) / 2)
    ...
    def image2_callback(self, data):
        ...
        self.target_position.data[2] = self.pix_to_m_ratio_img2 * ((np.absolute(self.target_history[2] - base_frame[1]) + sphere_relative_distance[1]) / 2)
        self.target_position_pub.publish(self.target_position)
